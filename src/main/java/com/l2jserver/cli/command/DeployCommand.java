...
public class DeployCommand extends AbstractCommand {
	...
	public void processZip(Path zipFilePath, Path dstDirPath) throws Exception {
		...
		env.put("create", "true");
		final var uri = URI.create("jar:file:" + zipFilePath.toAbsolutePath().toUri().getPath());
		try (var zipFs = FileSystems.newFileSystem(uri, env)) {
			...
			for (var p : zipFs.getRootDirectories()) {
	...
	public void processZipEntry(Path zipFsPath, Path dstDirPath) throws Exception {
		if (Files.isDirectory(zipFsPath)) {
			...
			final var outFilePath = Paths.get(dstDirPath.toString(), zipFsPath.toString());
			final var parentDirPath = outFilePath.getParent();
	...
	public static void checkZipSlip(Path dstDirPath, Path zipFsPath) throws Exception {
		final var dstFilePath = Paths.get(dstDirPath.toString(), zipFsPath.toString());
